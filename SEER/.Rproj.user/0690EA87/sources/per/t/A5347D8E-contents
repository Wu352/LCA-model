...
dat_surv[caus.death == "Lymphoma", status:= 1]
dat_surv[caus.death == "nonLymphoma", status:= 2]
dat_surv[, caus.death:= factor(caus.death, levels = c("Alive", "Lymphoma", "nonLymphoma"))]
...
dat_fit_aj[, group:= factor(group, levels = c(1, 2), labels = c("Lymphoma", "nonLymphoma"))]
...
fit_adj <- adjustedcif(data=dat_surv,
                       variable=igroup,
                       ev_time="surv.year",
                       event="surv.death", 
                       method="iptw",
                       cause=1,
                       treatment_model=fit_psm,
                       conf_int=FALSE)
...
vari_org <- c("age", "sex", "race", "site", "stage")
...
fit_adj2 <- adjustedcif(data=dat_surv,
                        variable=igroup,
                        ev_time="surv.year",
                        event="surv.death", 
                        method="iptw",
                        cause=1,
                        treatment_model=fit_psm2,
                        conf_int=FALSE)
...
dat_comp_group <- rbind(dat_fit_overall[, .(time, cif, strata, type)], dat_fit_aj[group == "Lymphoma", .(time, cif, strata, type)], dat_fit_iptw[, .(time, cif, strata, type)], dat_fit_iptw2[, .(time, cif, strata, type)])
...
cols_comp <- cols_sankey[c(7, 6, 8, 9)]
plot_cmp_adjust2 <- ggplot(data = dat_comp_group,
                           aes(x = time, y = cif, color = type)) +
...
plot_cmp_adjust2
ggsave(plot_cmp_adjust2, file = paste0("../outputs/", "plots/survival/CompFacet_cmprsk_", igroup, "_nclass", n_use, "_adjust.pdf"), dpi=600, width=vwidth, height=6, device="pdf")
