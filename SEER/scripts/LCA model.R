...
vari_lca <- c("age", "sex","race", "site", "stage")
...
jpeg(file = paste0("../outputs/", "lca/corrplot.jpeg"), res = 600, width = 10, height = 10, units = "in")
...
for (i in n_start:n_end) {
  ...
  save(mi_start, mi, file = paste0("lca/lca_m", i, ".RData"))
}
...
for (j in n_start:n_end) {
  ...
  if (file.exists(paste0( "lca/lca_m", j, ".txt"))) {
    p_file <- readLines(paste0("lca/lca_m", j, ".txt"))
    p_blrt <- as.numeric(strsplit(gsub("\"", "", p_file, fixed = FALSE), ": ", fixed = TRUE)[[1]][2])
  } else {
    if (run_blrt & j > 1) {
      p_blrt <- blrt_lca(vnum = j)
      sink(paste0("lca/lca_m", j, ".txt"))
