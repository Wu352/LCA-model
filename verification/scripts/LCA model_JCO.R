...
load("dat_cleaned_JCO.RData")
vari_lca <- c("age", "sex","ECOG", "site", "stage","tumor_size","LDH")
...
for (ivari in vari_lca) {
  ...
  for (jvari in vari_lca) {
    if (ivari == jvari) {
      ...
      jdat <- copy(dat_lca)
      setnames(jdat, c(ivari, jvari), c("xvari", "yvari"))名
...
colnames(res_corr) <- c("Age", "Sex","ECOG", "Site", "Stage","Tumor size","LDH")
rownames(res_corr) <- c("Age", "Sex","ECOG", "Site", "Stage","Tumor size","LDH")

jpeg(file = paste0("../outputs/", "lca/corrplot_JCO.jpeg"), res = 600, width = 10, height = 10, units = "in")
...
### Run poLCA
f_lca <- eval(parse(text = paste0("as.formula(cbind(", paste(vari_lca, collapse = ","), ") ~ 1)")))
...
for (i in n_start:n_end) {
  ...
  save(mi_start, mi, file = paste0("lca/lca_m", i, "_JCO.RData"))
}
...
for (j in n_start:n_end) {
  load(paste0("lca/lca_m", j, "_JCO.RData"))
  ...
  if (file.exists(paste0( "lca/lca_m", j, "_JCO.txt"))) {
    p_file <- readLines(paste0("lca/lca_m", j, "_JCO.txt"))
    p_blrt <- as.numeric(strsplit(gsub("\"", "", p_file, fixed = FALSE), ": ", fixed = TRUE)[[1]][2])
  } else {
    if (run_blrt & j > 1) {
      p_blrt <- blrt_lca(vnum = j)
      sink(paste0("lca/lca_m", j, "_JCO.txt"))
  ...
  res_j <- data.table(n.class = ncol(mi$posterior),
                      sample.size = mi$Nobs,
                      n.param = mi$npar,
                      loglike = mi$llik,
                      AIC = mi$aic,
                      BIC = mi$bic,
                      SABIC = sabic,
                      entropy = poLCA.entropy(mi),
                      p.blrt = p_blrt,
                      min.post = as.numeric(sum_post[1]),
                      median.post = as.numeric(sum_post[3]), 
                      mean.post = as.numeric(sum_post[4]),
                      max.post = as.numeric(sum_post[6]), 
                      mean.per.post = paste(round(mean_post, 2), 
                                            collapse = "|"), 
                      min.per.post = min(mean_post),
                      pre.prop = paste(round(pre_prop, 2),
                                       collapse = "|"), 
                      min.preprop = min(pre_prop), 
                      class.prop = paste(round(prop, 2), 
                                         collapse = "|"), 
                      min.prop = min(prop)
  )
...
res_lca[, p_lrt:= NA]
for (n in 2:n_end) {
  plrt <- calc_lrt(n = res_lca$sample.size[n],
                   null_ll = res_lca$loglike[n-1], 
                   null_param = res_lca$n.param[n-1],
                   null_classes = res_lca$n.class[n-1], 
                   alt_ll = res_lca$loglike[n],
                   alt_param = res_lca$n.param[n],
